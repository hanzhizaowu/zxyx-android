...
public class FeedFragment extends BaseFragment implements Presenter {
    ...
    private void getMoodList(int pageSize, Long feedId, boolean getLinear) {
        ...
        feedViewModel.getFeedPage(uid, feedId, Constants.PHOTO_TYPE_IMAGE).observe(this, new Observer<RetrofitResponseData<List<FeedDto>>>() {
            @Override
            public void onChanged(RetrofitResponseData<List<FeedDto>> response) {
                ...
                SPUtil.build().putLong(Constants.SP_FEED_ID,pageFeed.get(pageFeed.size() - 1).getFeedId());
